...
@Aspect
@Component
public class EmployeeControllerAop {
...
//	Logger logger =  LoggerFactory.getLogger(EmployeeControllerAop.class);
	...
	@Before(value = "execution(public * com.example.demo.controller.EmpoyeeController.*(..))")
	public void beforeAdviceforEmpController(JoinPoint jpoint) {
		LOGGER.info("Before Request goes to " + jpoint.getSignature() + " started at : " + new Date());
		LOGGER.debug(" request : " + jpoint.getArgs().toString());
	}

	@After(value = "execution(public * com.example.demo.controller.EmpoyeeController.*(..))")
	public void afterAdviceforEmpController(JoinPoint jpoint) {
...
//	}
	...
	@AfterThrowing(value = "execution(* com.example.demo.controller.EmpoyeeController.*(..))", throwing = "ex")
	public void afterThrowAdviceforEmpController(JoinPoint jpoint, Exception ex) {
		LOGGER.error(" AfterThrowing Responce goes to " + jpoint.getSignature() + " ended at : " + new Date());
		LOGGER.debug(" request : " + jpoint.getArgs());
		LOGGER.error("AfterThrowing " + jpoint.getSignature() + " error MSG: " + ex.getMessage() + " ended at : "
				+ new Date());
...
//	@Around(value = "execution(public * com.example.demo.controller.EmpoyeeController.addEmpoyee(..))")
//	public void aroundAdviceForEmpController(ProceedingJoinPoint pjpoint) {
...
//	}
	...
	// @Around print the logs request and responce

	@Around(value = "execution(public * com.example.demo.controller.EmpoyeeController.*(..))")
...
//			LOGGER.debug("Https Request Headers : " + 
			...
			request.getParameterMap().entrySet().stream().forEach(
					...
					entry -> {
						...
						LOGGER.debug("Key: " + entry.getKey() + "=> " + "Values: " + Arrays.toString(values));
